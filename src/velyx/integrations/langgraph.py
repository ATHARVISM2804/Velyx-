"""LangGraph integration for Velyx."""
...
# Setup logger
logger = logging.getLogger("velyx.integrations.langgraph")


class velyxIntegrationError(Exception):
    """Base exception for Velyx integration issues."""
...
class velyxLangGraphTools:
    """Adapter to expose Velyx as a set of Tools for LangGraph/LangChain agents.

    This class provides a bridge between the Velyx MemoryService and LangChain's
    tooling ecosystem.
    """

    def __init__(self, memory_service: MemoryService):
        """Initializes the velyxLangGraphTools with a memory service."""
    ...
    def save_memory_tool(self) -> StructuredTool:
        """Creates a tool to save information into Velyx."""
        ...
        async def _save(content: str, user_id: str, metadata: dict | None = None) -> str:
            ...
            filename = f"velyx_input_{uuid.uuid4()}.txt"
            ...
            try:
                ...
                return str(velyxIntegrationError(error_msg))
            finally:
    ...
    def search_memory_tool(self) -> StructuredTool:
        """Creates a tool to search for information in Velyx."""
