...
This document describes the self-hosted `velyx` Python package architecture as implemented in this repository.
...
Velyx follows the "memory as file system" concept from the README and implements it with three persistent layers:
...
`src/velyx/app/service.py` constructs and owns:
...
- `velyx.client.openai_wrapper`: opt-in OpenAI client wrapper that auto-retrieves memories and injects them into system context
- `velyx.integrations.langgraph`: LangChain/LangGraph tool adapter (`save_memory`, `search_memory`)
