# Velyx LangGraph Integration

The Velyx LangGraph Integration provides a seamless adapter to expose Velyx's powerful memory capabilities (`memorize` and `retrieve`) as standard [LangChain](https://python.langchain.com/) / [LangGraph](https://langchain-ai.github.io/langgraph/) tools. This allows your agents to persist information and recall it across sessions using Velyx as the long-term memory backend.
...
Here is a complete example of how to initialize the Velyx memory service and bind it to a LangGraph agent.

```python
import asyncio
import os
from velyx.app.service import MemoryService
from velyx.integrations.langgraph import velyxLangGraphTools

# Ensure you have your configuration set (e.g., env vars for DB connection)
# os.environ["velyx_DATABASE_URL"] = "..."
...
async def main():
    ...
    # Search for a memory
    search_tool = velyx_tools.search_memory_tool()
    print("\nSearching memory...")
...
```python
class velyxLangGraphTools(memory_service: MemoryService)
```
