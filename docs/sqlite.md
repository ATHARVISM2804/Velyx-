...
Velyx supports SQLite as a lightweight, file-based database backend for memory storage. This is ideal for:
...
# Using default SQLite file (velyx.db in current directory)
...
| `dsn` | `str` | `"sqlite:///velyx.db"` | SQLite connection string |
...
- **Relative path**: `sqlite:///./data/velyx.db`
- **Absolute path**: `sqlite:////home/user/data/velyx.db` (note the 4 slashes)

## Vector Search

SQLite doesn't have native vector support like PostgreSQL's pgvector. Velyx uses **brute-force cosine similarity** for vector search when using SQLite:
...
shutil.copy("velyx.db", "velyx_backup.db")
```
...
sqlite_config = DatabaseConfig(
    metadata_store={"provider": "sqlite", "dsn": "sqlite:///velyx.db"}
)
