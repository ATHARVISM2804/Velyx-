...
Welcome to Velyx! This guide will help you add robust long-term memory capabilities to your Python agents in just a few minutes. Without Velyx, LLMs are limited by their context window. Velyx solves this by providing an intelligent, persistent memory layer.

## Prerequisites

Before we begin, ensure you have the following:

-   **Python 3.13+**: Velyx takes advantage of modern Python features.
...
Velyx requires an LLM backend to function. By default, it looks for the `OPENAI_API_KEY` environment variable.
...
```python
"""
Getting Started with Velyx: A Robust Example.

This script demonstrates the core lifecycle of Velyx:
...
from velyx.app import MemoryService
...
async def main() -> None:
    """Run the Velyx lifecycle demonstration."""
    print(">>> Velyx Getting Started Example")
    print("-" * 30)

    # 1. API Key Handling
    # Velyx relies on an LLM backend (defaulting to OpenAI).
...
1.  **Initialization**: We configure `MemoryService` with specific `llm_profiles`. This tells Velyx which model to use. We also define a `memorize_config` with a "User Facts" category. Categories help the LLM organize and retrieve information more effectively.
...
3.  **Retrieval**: We use `retrieve` with a natural language query. Velyx's internal workflow ("RAG" or "LLM" based) will determine the best way to find relevant memories.
...
-   **Integrations**: Connect Velyx to your existing agent framework.

## Community Resources

This tutorial was created as part of the Velyx 2026 Challenge. For a summary of the architectural analysis, see the author's [LinkedIn Post](https://www.linkedin.com/posts/david-a-mamani-c_github-velyx-aivelyx-memory-infrastructure-activity-7418493617482207232-_MtG?utm_source=share&utm_medium=member_desktop&rcm=ACoAAFdc0CIB__DJovR2t1BOxxJ6tgEeOqVEgx4).
