"""
Test script to verify Nebius AI works with Velyx.
...
async def test_velyx_with_nebius():
    """Test Velyx with Nebius as the LLM provider."""
    ...
    print("\nTesting Velyx with Nebius provider...")

    # Configure Velyx to use Nebius (using dict config like example_1)
    llm_profiles = {
        "default": {
            "provider": "openai",
            "base_url": NEBIUS_BASE_URL,
    ...
    try:
        # Create Velyx service with Nebius
        ...
        print("\n" + "=" * 60)
        print("✓ SUCCESS: Velyx works with Nebius!")
        print("=" * 60)
        return True

    except Exception as e:
        print(f"  ✗ Velyx with Nebius failed: {e}")
        import traceback
...
async def main():
    print("=" * 60)
    print("Nebius AI + Velyx Integration Test")
    print("=" * 60)
    ...
    if chat_ok and embed_ok:
        # Test full Velyx integration
        await test_velyx_with_nebius()
    else:
        print("\n" + "=" * 60)
        print("✗ FAILED: Basic API tests failed, skipping Velyx test")
        print("=" * 60)
