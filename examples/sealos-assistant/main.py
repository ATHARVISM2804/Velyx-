...
async def lifespan(app: FastAPI):
    """Initialize Velyx memory service on startup."""
    global memory_service

    try:
        llm_profiles = get_llm_profiles()
        memory_service = MemoryService(llm_profiles=llm_profiles)
        print("✓ Velyx Memory Service initialized successfully")
    except Exception as e:
        print(f"✗ Failed to initialize Velyx: {e}")
        raise

    yield

    print("Shutting down Velyx Assistant...")


app = FastAPI(
    title="Velyx Assistant",
    description="AI Assistant with Long-Term Memory powered by Velyx",
    version="1.0.0",
    lifespan=lifespan,
)
...
if __name__ == "__main__":
    ...
    print(f"Starting Velyx Assistant on {host}:{port}")
