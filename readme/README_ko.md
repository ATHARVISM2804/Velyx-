...
velyx는 **24/7 프로액티브 에이전트**를 위해 구축된 메모리 프레임워크입니다.
장시간 실행을 위해 설계되었으며, 에이전트를 항상 온라인으로 유지하는 **LLM 토큰 비용을 크게 절감**하여 상시 가동되고 진화하는 에이전트를 프로덕션 시스템에서 실용적으로 만듭니다.
velyx는 **사용자 의도를 지속적으로 캡처하고 이해**합니다. 명령이 없어도 에이전트는 당신이 무엇을 하려는지 파악하고 스스로 행동합니다.
...
velyx는 **메모리를 파일 시스템처럼** 다룹니다—구조화되고, 계층적이며, 즉시 접근 가능합니다.
...
파일 시스템이 원시 바이트를 정리된 데이터로 변환하듯, velyx는 원시 상호작용을 **구조화되고, 검색 가능하며, 프로액티브한 인텔리전스**로 변환합니다.
...
velyx가 유용하거나 흥미롭다면, GitHub Star ⭐️를 눌러주시면 큰 힘이 됩니다.
...
velyx의 3계층 시스템은 **반응적 쿼리**와 **프로액티브 컨텍스트 로딩**을 모두 지원합니다:
...
👉 **[velyx.so](https://velyx.so)** - 7×24 지속 학습을 제공하는 호스팅 서비스

사용자 정의 프로액티브 워크플로우를 포함한 엔터프라이즈 배포는 **info@velyx.ai**로 문의하세요

#### 클라우드 API (v3)

| 기본 URL | `https://api.velyx.so` |
|---------|----------------------|
...
📚 **[전체 API 문서](https://velyx.pro/docs#cloud-version)**
...
velyx는 OpenAI 외에도 커스텀 LLM 및 임베딩 제공자를 지원합니다. `llm_profiles`로 구성:
```python
...
velyx는 [OpenRouter](https://openrouter.ai)를 모델 제공자로 지원하여 단일 API를 통해 여러 LLM 제공자에 액세스할 수 있습니다.
...
velyx는 **프로액티브 컨텍스트 로딩**과 **반응적 쿼리**를 모두 지원:
...
velyx는 모든 추론 작업에서 Locomo 벤치마크에서 **92.09% 평균 정확도**를 달성하여 신뢰할 수 있는 프로액티브 메모리 작업을 입증합니다.

<img width="100%" alt="benchmark" src="https://github.com/user-attachments/assets/6fec4884-94e5-4058-ad5c-baac3d7e76d9" />

상세 실험 데이터 보기: [Velyx-experiment](https://github.com/velyx-AI/Velyx-experiment)
...
| 리포지토리 | 설명 | 프로액티브 기능 |
|-----------|------|----------------|
| **[Velyx](https://github.com/velyx-AI/Velyx)** | 핵심 프로액티브 메모리 엔진 | 7×24 학습 파이프라인, 자동 분류 |
| **[Velyx-server](https://github.com/velyx-AI/Velyx-server)** | 지속 동기화가 포함된 백엔드 | 실시간 메모리 업데이트, 웹훅 트리거 |
| **[Velyx-ui](https://github.com/velyx-AI/Velyx-ui)** | 시각적 메모리 대시보드 | 라이브 메모리 진화 모니터링 |

**빠른 링크:**
- 🚀 [Velyx Cloud 체험](https://app.velyx.so/quick-start)
- 📚 [API 문서](https://velyx.pro/docs)
- 💬 [Discord 커뮤니티](https://discord.gg/velyx)
...
- **GitHub Issues**: [버그 보고 및 기능 요청](https://github.com/velyx-AI/Velyx/issues)
- **Discord**: [커뮤니티 참여](https://discord.com/invite/hQZntfGsbJ)
- **X (Twitter)**: [@velyx_ai 팔로우](https://x.com/velyx_ai)
- **연락처**: info@velyx.ai
